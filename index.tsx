...
import { 
  BookOpen, CheckCircle, XCircle, Award, ArrowLeft, RefreshCw, 
  FlaskConical, Globe, Calculator, Zap, Microscope, Mountain, 
  Image as ImageIcon, CheckSquare, ExternalLink, Circle, ArrowUpRight 
} from "lucide-react";
...
// --- Components: Background Shapes (From HeroGeometric) ---
...
// --- Components: Styled Button (Based on ButtonColorful) ---
interface StyledButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  active?: boolean;
  variant?: 'default' | 'option' | 'action';
  correctState?: 'correct' | 'incorrect' | null;
}

function StyledButton({ className, children, active, variant = 'default', correctState, ...props }: StyledButtonProps) {
  const baseStyles = "relative overflow-hidden transition-all duration-200 group border rounded-xl font-medium";
  
  const variants = {
    default: cn(
      "bg-zinc-900/50 hover:bg-zinc-800/80 border-white/[0.1]",
      active && "bg-indigo-500/20 border-indigo-500/50 text-indigo-200"
    ),
    option: cn(
      "text-left p-4 h-auto min-h-[60px]",
      "bg-white/[0.03] hover:bg-white/[0.08] border-white/[0.1]",
      active && !correctState && "bg-indigo-500/20 border-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.3)]",
      correctState === 'correct' && "bg-emerald-500/20 border-emerald-500 text-emerald-100 shadow-[0_0_15px_rgba(16,185,129,0.3)]",
      correctState === 'incorrect' && "bg-rose-500/20 border-rose-500 text-rose-100",
      !active && correctState === null && "text-zinc-300"
    ),
    action: cn(
      "bg-zinc-100 text-zinc-900 hover:bg-white border-transparent hover:scale-105 active:scale-95 shadow-lg shadow-white/10"
    )
  };

  return (
    <button className={cn(baseStyles, variants[variant], className)} {...props}>
      {/* Gradient Hover Effect for default/option */}
      {variant !== 'action' && !correctState && (
        <div className={cn(
          "absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500",
          "bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 blur-sm"
        )} />
      )}
...
const SUBJECT_CONFIG = {
  '化學': { icon: FlaskConical, color: 'text-purple-400' },
  '地科': { icon: Globe, color: 'text-blue-400' },
  '地理': { icon: Mountain, color: 'text-emerald-400' },
  '數學': { icon: Calculator, color: 'text-rose-400' },
  '物理': { icon: Zap, color: 'text-yellow-400' },
  '生物': { icon: Microscope, color: 'text-green-400' },
};

// Full Data extracted from original request
const MOCK_DATABASE = [
  // --- 化學 (1-25) ---
...
export default function App() {
  ...
  const [selection, setSelection] = useState({ grade: 1, semester: 1, exam: 2, subject: null as string | null });
  const [currentQuestions, setCurrentQuestions] = useState<typeof MOCK_DATABASE>([]);
    ...
    >
      <div className="text-center mb-16">
        ...
        >
          ...
          <span className="text-sm text-white/60 tracking-wide">HLHS Question Bank</span>
        </motion.div>
        ...
        <p className="text-white/40 text-lg font-light tracking-wide max-w-2xl mx-auto">
           Select your target exam and subject to begin the challenge.
        </p>
      </div>

      <div className="bg-white/[0.02] backdrop-blur-xl border border-white/[0.1] rounded-3xl p-8 shadow-2xl">
        {/* Filters */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {[
            { label: '年級', key: 'grade', options: [1, 2, 3], format: (v: number) => `${v} 年級` },
            { label: '學期', key: 'semester', options: [1, 2], format: (v: number) => v === 1 ? '上學期' : '下學期' },
            { label: '段考', key: 'exam', options: [1, 2, 3], format: (v: number) => `第 ${v} 次` }
          ].map((filter) => (
            <div key={filter.key} className="space-y-3">
              <label className="text-sm font-medium text-white/60 uppercase tracking-wider pl-1">{filter.label}</label>
              <div className="flex gap-2 p-1 bg-black/20 rounded-xl border border-white/[0.05]">
                {filter.options.map((opt) => {
                  // @ts-ignore
                  const isActive = selection[filter.key] === opt;
                  return (
                    <button
                      key={opt}
                      // @ts-ignore
                      onClick={() => setSelection({ ...selection, [filter.key]: opt })}
                      className={cn(
                        "flex-1 py-2.5 rounded-lg text-sm font-medium transition-all duration-300",
                        isActive 
                          ? "bg-white/[0.1] text-white shadow-lg shadow-indigo-500/10" 
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {Object.entries(SUBJECT_CONFIG).map(([subj, config]) => {
            const Icon = config.icon;
            return (
              <StyledButton
                key={subj}
                onClick={() => startQuiz(subj)}
                className="group p-6 h-32 flex flex-col items-center justify-center gap-4 hover:-translate-y-1"
              >
                <div className={cn("p-3 rounded-full bg-white/[0.05] group-hover:bg-white/[0.1] transition-colors", config.color)}>
                  <Icon size={32} strokeWidth={1.5} />
                </div>
                <span className="text-lg font-bold text-white/90 group-hover:text-white tracking-wide">{subj}</span>
  ...
  const renderQuiz = () => {
    ...
    return (
      <motion.div 
        ...
        className="max-w-4xl mx-auto px-4 relative z-10 w-full"
      >
        {/* Header */}
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
          <button 
            onClick={() => setStep('selection')} 
            className="flex items-center gap-2 text-white/40 hover:text-white transition-colors px-4 py-2 rounded-full hover:bg-white/[0.05]"
          >
            <ArrowLeft size={18} />
          ...
          <div className="flex flex-col items-center gap-1">
            <div className="text-xs font-medium text-white/40 uppercase tracking-widest">
              {selection.grade}年級 • {selection.subject} • 第{selection.exam}次段考
            </div>
            <div className="text-white/80 font-mono text-sm">
              Question {currentQIndex + 1} <span className="text-white/20">/</span> {currentQuestions.length}
            </div>
          </div>
          
          <a href={SOURCE_URL} target="_blank" rel="noopener noreferrer" className="hidden md:flex items-center gap-2 text-xs text-indigo-300 hover:text-indigo-200 bg-indigo-500/10 px-3 py-1.5 rounded-full border border-indigo-500/20 hover:bg-indigo-500/20 transition-all">
            <ExternalLink size={12} /> Source PDF
          </a>
        </div>

        {/* Progress Bar */}
        <div className="w-full bg-white/[0.05] rounded-full h-1 mb-8 overflow-hidden">
          <motion.div 
            initial={{ width: 0 }}
            animate={{ width: `${progress}%` }}
            className="bg-gradient-to-r from-indigo-500 to-rose-500 h-full shadow-[0_0_10px_rgba(99,102,241,0.5)]" 
          />
        </div>

        {/* Question Card */}
        <div className="bg-zinc-900/40 backdrop-blur-md border border-white/[0.1] rounded-3xl overflow-hidden shadow-2xl">
          <div className="p-8 md:p-10">
            <div className="flex items-start gap-4 mb-6">
              <span className={cn(
                "px-3 py-1 rounded-lg text-xs font-bold tracking-wider uppercase border",
                isMulti 
            ...
            <h2 className="text-xl md:text-2xl font-medium text-white/90 leading-relaxed mb-8">
              {question.question.replace('[需參考圖片]', '')}
            </h2>

            {question.hasImage && (
              <div className="w-full h-56 bg-black/30 rounded-2xl border-2 border-dashed border-white/[0.1] flex flex-col items-center justify-center text-white/30 gap-3 mb-8 hover:border-white/[0.2] transition-colors">
                <ImageIcon size={40} />
                <span className="text-sm">Image Reference Placeholder</span>
              </div>
            )}
            ...
            <div className="space-y-3">
              {question.options.map((opt, idx) => {
                ...
                let correctState: 'correct' | 'incorrect' | null = null;

                if (isAnswered) {
                  if (isCorrect) correctState = 'correct';
                  else if (isSelected) correctState = 'incorrect';
                }
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={cn(
                        "w-8 h-8 rounded-full border flex items-center justify-center text-sm font-bold transition-all",
                        isSelected 
                          ? "bg-indigo-500 text-white border-indigo-500" 
                          : "border-white/[0.2] text-white/40 group-hover/opt:border-white/[0.4] group-hover/opt:text-white/60",
                        correctState === 'correct' && "bg-emerald-500 border-emerald-500 text-white",
                        correctState === 'incorrect' && "bg-rose-500 border-rose-500 text-white"
                      )}>
                        {String.fromCharCode(65 + idx)}
                      </div>
                      <span className="text-base font-light tracking-wide">{opt}</span>
                    </div>
                    {isAnswered && (
                      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
                        {isCorrect && <CheckCircle className="text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.5)]" size={24} />}
                        {isSelected && !isCorrect && <XCircle className="text-rose-400 drop-shadow-[0_0_8px_rgba(251,113,133,0.5)]" size={24} />}
                      </motion.div>
                    )}
                  </StyledButton>
                );
            ...
            {isMulti && !isAnswered && (
              <div className="mt-8 flex justify-end">
                <StyledButton 
                  ...
                  className={cn(
                    "px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold tracking-wide transition-all shadow-lg shadow-indigo-500/20",
                    selectedOptions.length === 0 && "opacity-50 cursor-not-allowed bg-zinc-800 text-zinc-500 shadow-none"
                  )}
                >
                  <span className="flex items-center gap-2">
                    <CheckSquare size={18} /> Confirm Selection
                  </span>
                </StyledButton>
              </div>
            )}
          ...
          <AnimatePresence>
            {isAnswered && (
              <motion.div 
                ...
                className="bg-black/20 border-t border-white/[0.05]"
              >
                <div className="p-8 md:p-10">
                  <div className="flex gap-3 items-center mb-4">
                    <BookOpen size={20} className="text-indigo-400" />
                    <h3 className="font-bold text-lg text-white/90">Explanation</h3>
                  </div>
                  ...
                  <div className="mt-8 flex justify-end">
                    ...
                    >
                      {currentQIndex < currentQuestions.length - 1 ? 'Next Question →' : 'View Results'}
                    </StyledButton>
                  </div>
                </div>
              </motion.div>
  ...
  const renderResult = () => (
    <motion.div 
      ...
      animate={{ opacity: 1, scale: 1 }}
      className="max-w-md mx-auto relative z-10 px-6"
    >
      <div className="bg-zinc-900/40 backdrop-blur-xl border border-white/[0.1] rounded-[2rem] p-10 text-center shadow-2xl relative overflow-hidden">
        {/* Decorative glow */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 bg-indigo-500/20 blur-[50px] rounded-full" />
        
        <div className="relative">
          <div className="w-24 h-24 bg-gradient-to-tr from-indigo-500 to-rose-500 rounded-full flex items-center justify-center mx-auto mb-8 shadow-lg shadow-indigo-500/30">
            <Award size={48} className="text-white" />
          </div>
          
          <h2 className="text-3xl font-bold text-white mb-2">Quiz Complete</h2>
          <p className="text-white/40 mb-8">{selection.grade}年級 {selection.subject} 第{selection.exam}次段考</p>
          
          <div className="flex justify-center items-baseline gap-2 mb-10">
            <span className="text-7xl font-black bg-clip-text text-transparent bg-gradient-to-b from-white to-white/50">
              {score}
            </span>
            <span className="text-xl text-white/40 font-medium">/ {currentQuestions.length}</span>
          </div>
          
          <div className="space-y-4">
            <StyledButton 
              onClick={() => startQuiz(selection.subject!)}
              className="w-full py-4 bg-white text-black font-bold hover:bg-zinc-200"
            >
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#030303] text-zinc-100 font-sans selection:bg-rose-500/30">
